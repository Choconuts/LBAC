...
class Graph:
    ...
    def train(self, sess: tf.Session, ground_truth, iteration=-1, test=False, initialize=False):
        if iteration < 0:
            iteration = self.iter
        if initialize:
        ...
        if test:
            self.test_step(sess, ground_truth.get_test(), 1)
    ...
    def test_step(self, sess, batch, step):
        ...
        for e in self.evaluates:
            ...
            print('step {}, evaluation: {}'.format(step, e))
