...
from com.posture.skeleton import SMPLJoints, asf_smpl_map, smpl_asf_map
import com.posture.reader as reader
...
def load_motion(amc_file):
    motions = reader.parse_amc(amc_file)
    return motions


def translate_to_smpl_thetas(asf_file, motions):
    im = Imitator(
        reader.parse_asf(asf_file),
        SMPLModel(conf_path('smpl'))
    )
    thetas = []
    for i in range(len(motions)):
        theta = im.motion2theta(motions[i])
        thetas.append(theta)
...
if __name__ == '__main__':
    ...
    subject = '13_29'
    im = Imitator(
    reader.parse_asf(r'C:\Users\hp\Desktop\3 motion sequences\3 motion sequences\%s\%s.asf' % (subject, subject[0:2])),
    SMPLModel(conf_path('smpl'))
    )

    frame_idx = 0
    motions = reader.parse_amc(r'C:\Users\hp\Desktop\3 motion sequences\3 motion sequences\%s\%s.amc' % (subject, subject))
    theta = im.motion2theta(motions[frame_idx])
    print(join(conf_path('temp'), 'pose.npy'))
    np.save(join(conf_path('temp'), 'pose.npy'), theta)
    a = translate_to_smpl_thetas(r'C:\Users\hp\Desktop\3 motion sequences\3 motion sequences\%s\%s.asf' % (subject, subject[0:2]), motions[0:5])
    print(a.shape)
