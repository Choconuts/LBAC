...
class Mesh:
    ...
    def update_normal_only(self):
        # global global_tmp_mesh
        # global_tmp_mesh = self
        # p = ProcessPool(4)
        # p.map(process_vertex, range(len(self.vertices)))
        # p.close()
        # p.join()
...
global_tmp_mesh = None


def process_vertex(vi):
    fis = global_tmp_mesh.vertex_face_map[vi]
    vn = 0
    for fi in fis:
        vn += global_tmp_mesh.face_norm(global_tmp_mesh.faces[fi])
    vn /= np.linalg.norm(vn) + 0.0000001
    global_tmp_mesh.normal[vi] = vn
