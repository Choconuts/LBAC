...
class TstGroundTruth(GroundTruth):
    ...
    def get_batch(self, size):
        ...
        for i in range(1000):
            ...
            x.append([rd, rd + 1, rd + 2, rd + 3, rd + 4])
            y.append([2 * rd, 2 * rd + 2, 2 * rd + 4, 2 * rd + 6, 2 * rd + 8])
        xs = []
        ys = []
        ls = []
        for i in range(size):
            xs.append(x[self.ptr % len(x)])
            ys.append(y[self.ptr % len(y)])
            ls.append([5])
            self.ptr += 1
...
class PoseGroundTruth(GroundTruth):
    ...
    def gen_truth(self, results_dir, wait=21, stride=5, n_pose=20, gen_ranges=None, no_rotation=False):
        ...
        for i in gens:
            ...
            for i in range(act_len):
                out_meshes[i], out_poses[i] = self.pre_process(out_meshes[i], out_poses[i], no_rotation)
    ...
    def pre_process(self, mesh, pose, flag):
        ...
        mesh.update()
        if flag:
            pose = self.remove_rotation(pose)
        return mesh, pose
